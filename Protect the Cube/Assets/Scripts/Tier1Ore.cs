...
public class Tier1Ore : MonoBehaviour
{
    ...
    private int oreT1ExpDropAmt; // stores # of exp orbs dropped on die()
    private int oreT1GoldDropAmt;// stores # of gold orbs dropped on die()
    ...
    [SerializeField] public float hpBarVerticalOffset = 2.0f;
    ...
    void Start()
    {
        ...
        currentHealth = GameManager.Instance.OreManager.ORE_T1_MAX_HEALTH;
        oreT1ExpDropAmt = GameManager.Instance.OreManager.ORE_T1_DROP_XP;
        oreT1GoldDropAmt = 0; // no gold for Tier 1 Ores
        ...
        if (capsuleTransform != null)
        {
            ...
            foreach (Transform eye_transform in capsuleTransform)
            {
                ...
                if (num_eyes_deactivated > (total_num_eyes - GameManager.Instance.OreManager.ORE_T1_NUM_EYES)) { break; }
    ...
    public void DropExp(){
        for (int i = 0; i < oreT1ExpDropAmt; i++)
        {
    ...
    {
        for (int i = 0; i < oreT1GoldDropAmt; i++)
        {
    ...
    public void UpdateHPBar() { if (hpBar != null) hpBar.value = currentHealth / GameManager.Instance.OreManager.ORE_T1_MAX_HEALTH; }
